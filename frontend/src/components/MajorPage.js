...
import Service from '../services/majors'
import Table from './MajorTable'
import Form from './MajorForm';
...
const MajorPage = (props) => {
	...
	const handleDeleteTupleObj = async(tupleObj_id) => {
		try {
			...
			const newTable = await tupleObjs.filter(tupleObj => {
				return tupleObj.major_id !== tupleObj_id;
			});
	...
	const handleUpdateTupleObj = async(oriTupleObjId, tupleObj) => {
		try {
			...
			const newTable = await tupleObjs.map(tuple => {
				return tuple.major_id !== oriTupleObjId ? tuple : tupleObj;
			});
	...
	return (
		<div>
			...
			<div>
				<Togglable ref={togglableRef} buttonLabel='Insert Major' type='Form'>
