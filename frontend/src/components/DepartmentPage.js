...
import Service from '../services/departments'
...
const DepartmentPage = (props) => {
	...
	const handleDeleteTupleObj = async(tupleObj_id) => {
		try {
			...
			const newTable = await tupleObjs.filter(tupleObj => {
				return tupleObj.department_id !== tupleObj_id;
			});
	...
	const handleUpdateTupleObj = async(oriTupleObjId, tupleObj) => {
		try {
			...
			const newTable = await tupleObjs.map(tuple => {
				return tuple.department_id !== oriTupleObjId ? tuple : tupleObj;
			});
	...
	return (
		<div>
			...
			<div>
				<Togglable ref={togglableRef} buttonLabel='Insert Department' type='Form'>
